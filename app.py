...
try:
    ...
    response = requests.get("https://smeplug.com/api/v1/balance", headers=headers)
    
    if response.status_code == 200:
        st.success("✅ API YA HAUBA - An haɗa da SMEPlug")
        user_data = response.json()
        st.write(f"Sannu, {user_data.get('username', 'Jamilu')}")
        st.write(f"Balance: ₦{user_data.get('balance', '0')}")
    else:
    ...
    st.warning("Duba API Key dinka a Secrets")
