...
st.set_page_config(page_title="J.S.GLOBAL", page_icon="💳", layout="wide", initial_sidebar_state="collapsed")
...
def save_transaction(user_id, service, amount, status, ref, phone=""):
    try:
        supabase.table('transactions').insert({
            'user_id': user_id, 'service': service, 'amount': amount, 'status': status,
            'reference': ref, 'phone': phone, 'date': datetime.now().isoformat()
        }).execute()
    except:
        pass
...
def add_commission(reseller_id, sub_user_id, service, amount):
    try:
        ...
        supabase.table('commissions').insert({
            'reseller_id': reseller_id, 'sub_user_id': sub_user_id, 'service': service,
            'amount': amount, 'commission': commission, 'date': datetime.now().isoformat()
        }).execute()
...
if not st.session_state.logged_in:
        ...
        if st.button("Sign Up", type="primary", use_container_width=True):
            try:
                supabase.table('users').insert({
                    'full_name': name, 'email': signup_email, 'password': signup_pass, 'phone': signup_phone,
                    'wallet_balance': 0, 'kyc_status': 'Not Submitted', 'account_type': 'user',
                    'commission_rate': 0, 'total_earnings': 0
                }).execute()
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        menu_items = [("🏠", "Dashboard"), ("👤", "KYC Verification"), ("👥", "My Resellers"), ("💰", "Fund Wallet"), ("📱", "Buy Airtime"), ("📶", "Buy Data"), ("📺", "Cable TV"), ("⚡", "Electricity"), ("💸", "Transactions"), ("👑", "Admin Panel")]
    ...
    if st.session_state.page == "Dashboard":
        ...
        if st.session_state.account_type == "reseller":
            st.markdown(f"<div class='upgrade-card'><div style='display: flex; justify-content: space-between; align-items: center;'><div><div style='font-weight: 700; font-size: 14px; color: #000;'>Reseller Account</div><div style='font-size: 12px; color: #666; margin-top: 2px;'>Total Earnings: ₦{user_data['total_earnings']:,.2f}</div></div><div style='color: #1976D2; font-size: 20px;'>›</div></div></div>", unsafe_allow_html=True)
        ...
        if user_data['account_type'] == 'reseller':
            ...
            with tab2:
                with st.form("sub_user_form"):
                    ...
                    if st.form_submit_button("Create Sub-User", type="primary"):
                        try:
                            supabase.table('users').insert({
                                'full_name': sub_name, 'email': sub_email, 'password': sub_pass, 'phone': sub_phone,
                                'account_type': 'sub_user', 'parent_id': st.session_state.user_id, 'wallet_balance': 0,
                                'kyc_status': 'Not Submitted', 'commission_rate': 0, 'total_earnings': 0
                            }).execute()
            ...
            if st.button("Upgrade to Reseller - ₦10,000", type="primary"):
                if st.session_state.wallet_balance >= 10000:
                    ...
                    supabase.table('users').update({'account_type': 'reseller', 'commission_rate': 2.0, 'wallet_balance': new_bal}).eq('id', st.session_state.user_id).execute()
        ...
        if st.button("Pay with Paystack", type="primary", use_container_width=True):
            ...
            data = {"email": st.session_state.current_user, "amount": int(amount * 100), "callback_url": "https://yourapp.streamlit.app"}
